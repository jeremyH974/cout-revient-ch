...
| #   | Proposition                                                                                                            | Valeur | Fiabilité | Satisf. | Sessions |  ROI   |   Phase   |
| --- | ---------------------------------------------------------------------------------------------------------------------- | :----: | :-------: | :-----: | :------: | :----: | :-------: |
| P27 | « Copier un résumé anonymisé de mes stats » pour une revue assistée par l'IA de son choix                              |   2    |     1     |    3    |   0,5    | **12** |    T3     |
| P23 | Bouton « Actualiser » v2 : horodatage, source, badge « périmé », prix Hyperliquid/DefiLlama, clé CoinGecko optionnelle |   3    |     4     |    4    |    1     | **11** | immédiat  |
| P28 | Vue d'ensemble v2 : valeur nette consolidée, cartes des deux espaces, flux entre espaces, alertes                      |   4    |     3     |    5    |   1,5    |   8    |    T3     |
| P22 | Statistiques de trading : espérance, profit factor, drawdown, ventilations, calendrier, garde-fous, export             |   5    |     2     |    4    |   2,5    |  4,4   |    T3     |
| P21 | Journal de trading : aller-retours automatiques, saisie manuelle, notes, tags/setups, plan → R                         |   5    |     2     |    5    |    3     |   4    |    T2     |
| P19 | Espaces Investissement / Trading + comptes de première classe + navigation v2 + Vue d'ensemble v1                      |   4    |     3     |    4    |    3     |  3,7   |    T0     |
| P20 | Import Hyperliquid en lecture seule (adresse publique) : fills spot + perps, funding, dépôts/retraits                  |   5    |     3     |    5    |    4     |  3,3   |    T1     |
| P24 | Import « format pivot » (CSV Universal Koinly/Waltio) + virements internes appariés + convertisseurs natifs            |   4    |     3     |    3    |   3 +    |  3,3   |    T4     |
| P26 | Mode « live » WebSocket optionnel sur l'écran Trading                                                                  |   2    |     1     |    2    |   1,5    |  3,3   | plus tard |
| P25 | Lecture on-chain par adresse (BTC mempool.space, EVM Etherscan V2 / Blockscout)                                        |   2    |     2     |    3    |    3     |  2,3   | plus tard |

P12 (sauvegarde robuste, IndexedDB) devient un **prérequis** de P20 (volume des fills Hyperliquid).
**P17 est remplacé par P24** (format pivot + convertisseurs à la demande, jamais de clé API
d'exchange dans le site).
...
→ **P19** → **P12** → **P20** → **P21** → **P28** → **P22 + P27** → **P24** → P10 → P25/P26 si
demande. Chaque étape est utilisable seule ; la V1 reste déployée tant que la V2 n'est pas prête.
...
- **P24 — Format pivot (3 sessions + 0,5 par convertisseur)** : CSV Universal Koinly/Waltio, JSON
  Ghostfolio, virements internes appariés (report de coût), convertisseurs Kraken / Coinbase /
  Bitvavo / Revolut / Ledger Live à la demande, fixtures synthétiques obligatoires.
