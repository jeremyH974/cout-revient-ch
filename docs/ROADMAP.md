...
| P110 | **Retournée par la mesure, et livrée autrement (14/09/2026, décision n° 151).** Le calcul hors du fil qui dessine l'écran |   2    |     2     |    4    |   1,5    | **5,3** |
| P108 | **Livré (13/09/2026, décision n° 150).** L'arbitrage PFU / barème, chiffré                                                |   5    |     3     |    5    |   2,5    | **5,2** |
| P114 | **Livré (14/09/2026, décision n° 151).** Aligner le chiffrement de la sauvegarde exportée                                 |   0    |     2     |    0    |   0,5    |  **4**  |
...
- **P107 est livrée, et son écriture a corrigé le dépôt trois fois** (décisions n° 148 et 149). Le
  vocabulaire des cases a d'abord dû être inventé : quatre registres divergents, aucun type commun,
  et les cases crypto absentes du dépôt entier. Puis l'écran a obligé à relire les imprimés
  officiels, qui ont démenti le code sur **trois** points : 2TT, 2CK et 2CG sont sur la 2042 et
  non sur la 2042 C — une erreur qu'aucun test ne pouvait voir, faute d'un champ `form` obligatoire ;
...
- **P110 visait le fil d'exécution ; la mesure a montré que le coût était ailleurs** (décision
  n° 151). Un Web Worker aurait rendu 29 secondes **animées** au lieu de **figées**, sans en
  raccourcir une seule. Le grand livre était **rejoué trois fois pour un seul écran** : le jouer
...
- **P114 a renversé un arbitrage de la décision n° 120**, pour la même raison que P113 avant
  ...
  gardait la serrure la plus faible.
...
| P64  | Qualification assistée des lignes inconnues (déterministe, IA en appoint) |   5    |     4     |    4    |    3     |  4,3   | **livré (30/08/2026)** |
